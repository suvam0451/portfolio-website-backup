...
Where to **_not use_** ISMC:

- Where we need _organic fade-in/fade-out_ (e.g.- pebbles, water ripples, particles)
- Destructible areas (Although it can be implemented(switched with destructible in runtime), the player might notice the glitchy switch)
- If performance cost is not major issue(non-mobile), _houdini assets_ actually can give us better variation for same effort. regardless, it is always useful to know both.

Where to **_use_** ISMC:

- Static/non-destructible environments: (House interior, cave systems, fences, walls)
- Procedural objects: (Simulaing huge towns, procedural dungeons)
- Learning Experience: The series covers some interesting ways to design your level. Always a plus to know and share more.

In this first part of the series, we will make a hall with pillars and a floor grid.

##### (~~Promo~~) Before we start

I am working on a **_vscode extension_** that provides code snippets, terminal tools(_such as shader compile_) and some GUI goodness.
If you use vscode, please check the last section. It is free and community driven. Thanks !!!
...
![Properties](../../../images/instanced-static-meshes/Promo01.gif)

<GifOverlay ImageLink="../../../images/instanced-static-meshes/Promo01.gif">
