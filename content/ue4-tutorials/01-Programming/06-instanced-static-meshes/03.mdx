...
In the previous part, **_alternately spawn Instanced meshes_** using a map of ISMC.
In this part, we are going to add a **_pseudo-random number provider_** and set the meshes to use their original length.
For that, we would have to use a struct instead of StaticMesh reference. Also, we need to deal with that hard reference problem.

We will be doing it in this order: 
1. Refactoring our utility functions.
2. Introduce soft asset pointers *(lazy loading for aother day)*
3. Getting the new overlay ready for designer.
4. Adding Pseudo-Random provider.
5. Playing around with Semi-procedural wall we made.

---

The utility function in question is
```cpp
// cleanup
...
I actually found a much better way to implement this. Please copy over the following code to any library file you want to have it in.
It is useful in multiple situations. It is completely fine if you don't understand about templates.
Just keep in mind the second following snippets to see how to implement it.

```cpp
// Lets say it is in Library.h. The following functions are for Attached Components, Reference Arrays, Reference maps...
...
From now, I am going to assume that you have this function in classes you create(Or you can store it in your library and include it).
If you have any trouble, open up te sample project I sent and refer. This is important.
